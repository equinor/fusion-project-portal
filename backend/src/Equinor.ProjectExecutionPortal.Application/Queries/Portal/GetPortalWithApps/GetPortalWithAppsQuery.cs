...
public class GetPortalWithAppsQuery : QueryBase<PortalDto?>
{
    ...
    public class Handler : IRequestHandler<GetPortalWithAppsQuery, PortalDto?>
    {
        ...
        private async Task TempMethodSeedDb(CancellationToken cancellationToken)
        {
            ...
            var collaborationAppGroup = new Domain.Entities.AppGroup("Collaboration", 0, "#E24973");
            var projectInformationAppGroup = new Domain.Entities.AppGroup("Project Information", 1, "#6D2FD5");
            var ccAppGroup = new Domain.Entities.AppGroup("Construction and Commissioning", 2, "#0084C4");
            var demoAppGroup = new Domain.Entities.AppGroup("Demo", 3, "#00977B");
            ...
            var reviewsApp = new Domain.Entities.OnboardedApp("reviews", 1);
            ...
            projectInformationAppGroup.AddApp(tasksApp);
            projectInformationAppGroup.AddApp(orgChartApp);
            ccAppGroup.AddApp(handoverGardenApp);
            ccAppGroup.AddApp(workOrderGardenApp);
            demoAppGroup.AddApp(demoApp);
            ...
            var orgChartWsApp = new WorkSurfaceApp(orgChartApp.Id, workSurface2.Id);
            var handoverGardenWsApp = new WorkSurfaceApp(handoverGardenApp.Id, workSurface2.Id);
            var workOrderGardenWsApp = new WorkSurfaceApp(workOrderGardenApp.Id, workSurface2.Id);
            var demoWsApp = new WorkSurfaceApp(demoApp.Id, workSurface2.Id);

            await _context.Set<WorkSurfaceApp>().AddRangeAsync(meetingsWsApp, reviewsWsApp, tasksWsApp, orgChartWsApp, handoverGardenWsApp, workOrderGardenWsApp, demoWsApp);
