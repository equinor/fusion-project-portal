...
export class LegacyAuthContainer extends AuthContainer {
  ...
  public async requiresAuth(): Promise<void> {
    ...
    const valid =
      account &&
      (account.idTokenClaims as { exp: number })?.exp > Date.now() / 1000;
    if (!valid) {
      try {
  ...
  async loginAsync(clientId: string): Promise<void> {
    ...
    DEBUG_LOG &&
      console.trace(
        `FusionAuthContainer::loginAsync for client id [${clientId}]`
      );
  ...
  async registerAppAsync(
    ...
    if (!isRegistered && legacy) {
      DEBUG_LOG && console.warn(`registering legacy client for [${clientId}]`);
