import { Chip, Icon, Tooltip } from "@equinor/eds-core-react";
...
const getColor = (messages: ServiceMessage[]): string => {
    return messages.some(message => message.type === "Issue") ? tokens.colors.interactive.danger__resting.rgba : messages.some(message => message.type === "Maintenance") ? tokens.colors.interactive.warning__resting.rgba : tokens.colors.ui.background__info.rgba;
}
...
export function ServiceMessageTooltip() {
    ...
    return (
        <div>
            <p>Shows relevant messages for <b>Project Portal</b> system status</p>
            <StyledIndicators>
                <StyledIndicator>
...
export function ServiceMessageIcon() {
    ...
    return (<>
        {count > 0 && <StyledMessageChip color={color}>{count}</StyledMessageChip>}
