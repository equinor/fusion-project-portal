...
export function setContextHistory(context: ContextHistoryItem) {
	const storedContextHistory = storage.getItem<ContextHistoryItem[]>(CONTEXT_HISTORY_SOCAGE_KEY) || [];
	...
	if (currentContextIndex !== -1) {
		storage.setItem(CONTEXT_HISTORY_SOCAGE_KEY, moveItemToTopByIndex(storedContextHistory, currentContextIndex));
	} else {
		const contextHistory = [context, ...storedContextHistory];

		storage.setItem(CONTEXT_HISTORY_SOCAGE_KEY, contextHistory.slice(0, getContextHistoryLength() - 1));
	}
}
...
export function getContextHistoryLength(): number {
	const contextHistoryLength = storage.getItem<string>(CONTEXT_HISTORY_LENGTH_KEY) || CONTEXT_HISTORY_LENGTH;

	return Number(contextHistoryLength) > 5 ? 5 : Number(contextHistoryLength);
}
function moveItemToTopByIndex(storedContextHistory: ContextHistoryItem[], currentContextIndex: number): any {
