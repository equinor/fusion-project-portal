...
const PortalCard = ({ portal, onQuickEdit }: { portal: Portal; onQuickEdit: (portal: Portal) => void }) => {
	...
	return (
		...
		>
			<Style.Card active={portal.id === activePortalId ? colors.interactive.primary__selected_highlight.hex : ''}>
				<Style.IconWrapper>
				...
				<Style.Content>
					...
					<Style.Description variant="caption">Description: {portal.description}</Style.Description>
				...
				>
					...
					>
						<Icon data={assignment} size={16} color={colors.text.static_icons__tertiary.hex} />
	...
	return (
		<Style.CardList>
			{portalsData?.map((portal) => (
				<PortalCard key={portal.id} portal={portal} onQuickEdit={(portal) => setQuickEdit(portal)} />
			))}
